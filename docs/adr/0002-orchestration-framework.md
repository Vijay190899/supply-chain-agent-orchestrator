# ADR-0002: Orchestration framework — LangGraph, and a CrewAI comparison
...
Separately, re-implement the same four-role workflow in **CrewAI** and publish a written comparison: development effort, control granularity, how each handles the human-in-the-loop interrupt, and token cost. CrewAI is the natural foil because it's higher-level — the comparison measures exactly that trade-off.
